...
bot.on("messageReactionAdd", (message, emoji, userId)=>{
	...
	if(userId != bot.user.id) {
		if(emoji.name=="👍") {
			...
			bot.getDMChannel(bot.users.get(message.embeds[0].description.split('>')[0].split('<@')[1]).id)
				.then((user)=>{	
			...
			bot.getDMChannel(bot.users.get(message.embeds[0].description.split('>')[0].split('<@')[1]).id)
				.then((user)=>{
					user.createMessage('You have been banned from posting in <#751301214175625216> due to Spam/Abuse.')
						.catch((e)=>{
							bot.createMessage(message.channel.id, "Direct messages are off for "+message.embeds[0].description.split(' ')[0]+".");
						})
				});
